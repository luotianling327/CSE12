...
public:	
	...
	cp .cproject .project .hw3.vcproj .hw3.sln .hw3.suo Makefile \
	...
	chmod 644 .cproject .project .hw3.vcproj .hw3.sln
	#
	...
	#
	# set correct permissions on solution source files (after hw2 due date)
	...
	if [ -d $(HOME)/public_html/hw3/ ]; then rm -r $(HOME)/public_html/hw3/; fi
	mkdir $(HOME)/public_html/hw3
	cp *html $(HOME)/public_html/hw3
